...
- 📸 **Автоматическая обработка изображений** - оптимизация и сжатие изображений из markdown и GitHub
- 🚨 **Красивые страницы ошибок** - 10 анимированных страниц (400-404, 500-504) с глитч-эффектом и прикольными слоганами
- 📋 **Новый формат doc-config** - компактный и читаемый синтаксис для определения структуры сайта (см. [DOC_CONFIG_SYNTAX.md](docs/DOC_CONFIG_SYNTAX.md))
...
```bash
# Сборка из папки website (аргумент обязателен!)
node build-all-v3.js website

# Или через npm (аргумент тоже обязателен!)
...
**⚠️ Важно:** Аргумент с путем к папке сайта всегда обязателен!

### 3. Структура папки сайта

Каждая папка сайта должна содержать:

- **config.yaml** - настройки сайта (название, навигация, стили)
- **doc-config.yaml** - структура документации (иерархия страниц)
- **home.md** - главная страница (станет index.html)
...
### 📚 Полный индекс
- **[DOCUMENTATION_INDEX.md](DOCUMENTATION_INDEX.md)** - полный список всей документации (27 файлов)

### 🚀 Быстрый доступ
- **[ГОТОВО.md](ГОТОВО.md)** - краткая сводка изменений v3 и готовность к использованию
- **[QUICK_REFERENCE.md](QUICK_REFERENCE.md)** - быстрая справка по командам и примерам

### Руководства для начинающих
- **[БЫСТРЫЙ_СТАРТ.md](БЫСТРЫЙ_СТАРТ.md)** - краткое руководство на русском
...
- **[BUILD_USAGE.md](BUILD_USAGE.md)** - полное руководство по использованию системы сборки
...
- **[DOC_CONFIG_SYNTAX.md](docs/DOC_CONFIG_SYNTAX.md)** - полное описание синтаксиса doc-config.yaml
- **[DOC_CONFIG_CHEATSHEET.md](docs/DOC_CONFIG_CHEATSHEET.md)** - шпаргалка по doc-config.yaml
...
# 3. Вернитесь в корень и соберите (аргумент обязателен!)
cd ..
node build-all-v3.js my-site
# или
npm run build my-site
...
Подробнее: [CREATE_NEW_SITE.md](CREATE_NEW_SITE.md)

## 📋 Новый формат doc-config.yaml

Версия 3 использует компактный и читаемый формат:
...
- Тип определяется по значению (файл/папка/URL)
- Более читаемая структура

**Документация:**
- [DOC_CONFIG_QUICKSTART.md](docs/DOC_CONFIG_QUICKSTART.md) - быстрый старт
- [DOC_CONFIG_FORMAT.md](docs/DOC_CONFIG_FORMAT.md) - полное описание формата
- [DOC_CONFIG_EXAMPLES.md](docs/DOC_CONFIG_EXAMPLES.md) - примеры использования
- [MIGRATION_GUIDE.md](docs/MIGRATION_GUIDE.md) - миграция со старого формата
...
| Команда | Описание |
|---------|----------|
| `npm run build` | Основная сборка (build-all-v3.js + страницы ошибок) |
...
| `npm run errors:generate` | Генерация только страниц ошибок |
...
- 📸 [Обработка изображений](ОБРАБОТКА_ИЗОБРАЖЕНИЙ.md) - автоматическая оптимизация изображений (RU)
- 📸 [Image Processing](docs/IMAGE_PROCESSING.md) - automatic image optimization (EN)
